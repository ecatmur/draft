...
non-modifying for some operand, additional variations of that
expression that accept differing const-qualification and/or value category for
...
\begin{example}
\begin{codeblock}
template<class U> concept D = requires(U a, U b) { a <=> b; };
struct U {
    int i;
    auto operator<=>(U& r) { return i <=> r.i; }
    auto operator<=>(const U& r) const { return r.i <=> i; }
};
\end{codeblock}

\tcode{U} meets the (explicit and implicit) syntactic requirements of \tcode{D}
but does not meet its implicit semantic requirements, so a program requiring
\tcode{D<U>} is ill-formed, no diagnostic required.
\end{example}
